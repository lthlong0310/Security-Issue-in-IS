...
namespace UI
{
    public partial class LoginForm : Form
    {
        ...
        {
            User current_user = new User(usernametb.Text, passwordtb.Text);
            current_user = new BLL.BLL().Login(current_user);
            this.Hide();
            if (current_user.usertype == "svien")
            {
                SinhVienMenuForm svmenu = new SinhVienMenuForm(current_user);
                svmenu.ShowDialog();
            }
            else if (current_user.usertype == "gvien")
            {
                GiaoVienMenuForm gvienmenu = new GiaoVienMenuForm(current_user);
                gvienmenu.ShowDialog();
            }
            else if (current_user.usertype == "gvu")
            {
                GiaoVuMenuForm gvumenu = new GiaoVuMenuForm(current_user);
                gvumenu.ShowDialog();
            }
            else if (current_user.usertype == "trgbm")
            {
                TruongBoMonMenuForm trgbmmenu = new TruongBoMonMenuForm(current_user);
                trgbmmenu.ShowDialog();
            }
            else if (current_user.usertype == "trgkhoa")
            {
                TruongKhoaMenuForm trgkhoamenu = new TruongKhoaMenuForm(current_user);
                trgkhoamenu.ShowDialog();
            }
            else
            {
