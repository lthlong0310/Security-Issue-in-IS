...
IS
    ...
    ENCRYPTED_DATA RAW(1000);
BEGIN
    ENCRYPTED_DATA := DBMS_CRYPTO.ENCRYPT(
        SRC => CONVERT(UTL_RAW.CAST_TO_RAW(INPUT_DATA),'US7ASCII','AL32UTF8'),
        TYP => ENCRYPTION_TYPE,
        KEY => UTL_RAW.CAST_TO_RAW(MY_KEY)
    );
...
BEGIN
    ...
    RETURN CONVERT(UTL_RAW.CAST_TO_VARCHAR2(DECRYPTED_DATA),'US7ASCII','AL32UTF8');
END DECRYPT_;
/
