...
--LIST OUT TABLE NAME
SELECT * FROM DBA_TABLES WHERE OWNER = 'SYS' AND LAST_ANALYZED like TO_DATE('20200604', 'yyyymmdd') and
...
--NO GRANT OPTION
CREATE OR REPLACE PROCEDURE P_UPDATE_NO(COL_NAME IN VARCHAR2, USR_NAME IN VARCHAR2)
AS
    BEGIN
        EXECUTE IMMEDIATE 'GRANT UPDATE ON'|| COL_NAME || 'TO' || USR_NAME;
    end;
--WITH GRANT OPTION
...
AS
    BEGIN
        EXECUTE IMMEDIATE 'GRANT UPDATE ON'|| COL_NAME || 'TO' || USR_NAME ||'WITH GRANT OPTION';
    end;
